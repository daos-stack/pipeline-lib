...
Void call(Map config= [:]) {
   ...
   */
    ...
    if (config['junit_files'] && config['result'] != 'FAILURE') {
        ...
        Map stageFlowNode = null
        for (Map s in stage['stageFlowNodes']) {
            if (s['name'] == flow_name) {
                stageFlowNode = s
                break
            }
        ...
        if (env.CHANGE_ID) {
            ...
            if (commitPragma('Skip-PR-comments').toLowerCase() == 'true') {
                emailext subject: "${JOB_NAME} ${config.name} stage ${config.result}",
                         ...
                         'in error.\n\n' +
                         'Please accept my apologies and kindly forward this message to\n' +
