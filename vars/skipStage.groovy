...
boolean skip_ftest_valgrind(String distro, String target_branch) {
    ...
    if (!skip_stage_pragma('func-test-vm-valgrind', 'true')) {
        println('in skip_ftest_valgrind, NOT skipping')
        return false
    }

    params_value = paramsValue('CI_FUNCTIONAL_' + distro + '_VALGRIND_TEST', false)
    println('in skip_ftest_valgrind, params_value = ' + params_value)
    if (!params_value) {
        println('in skip_ftest_valgrind, skipping')
        return true
    }
