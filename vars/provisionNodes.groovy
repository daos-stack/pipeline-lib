...
def call(Map config = [:]) {
  ...
  sshagent (credentials: ['daos-provisioner']) {
    ...
    def provision_script = '''set -ex
                              ...
                              for ext in \\$pkgs; do
                                  rm -f /etc/yum.repos.d/build.hpdd.intel.com_job_daos-stack_job_\\"\\${ext}\\"_job_*.repo
                                  yum-config-manager --add-repo=http://build.hpdd.intel.com/job/daos-stack/job/\\"\\${ext}\\"/job/master/lastSuccessfulBuild/artifact/artifacts/
