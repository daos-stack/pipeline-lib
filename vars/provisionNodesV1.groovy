/* groovylint-disable DuplicateNumberLiteral, DuplicateStringLiteral, LineLength, VariableName */
// vars/provisionNodesV1.groovy

/**
 * provisionNodesV1.groovy
 *
 * provisionNodesV1 variable
 */
...
void call(Map config = [:]) {
    ...
    List<String> node_list = config['NODELIST'].split(',')
    ...
    String cleanup_logs = 'clush -B -l root -w ' + nodeString +
...
EOF'''
    ...
    if (distro.startsWith('el7')) {
        ...
        if (repository_g != '') {
            String repo_file = repository_g.substring(
        ...
        if (repository_l != '') {
            String repo_file = repository_l.substring(
        ...
        error('Don\'t know how to handle repos for distro/distro_type: ' + distro + '/' + distro_type)
    }
    /* groovylint-disable-next-line GStringExpressionWithinString */
    provision_script += '''\nsync; sync"
                           exit ${PIPESTATUS[0]}'''
